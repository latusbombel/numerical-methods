...
double funkcja(double alpha, double beta, double y, double (*f)(double alpha, double beta, double y)){
    return f(alpha, beta, y);
}

double f(double alpha, double beta, double y){
...
int main(){
    ...
    for(int i = 0; ; i++){
        y.push_back(y[i]);
        y[i+1] = met_picarda(y, max_iter, t, i, funkcja(alpha, beta, y[i], f), funkcja(alpha, beta, y[i+1], f), TOL);
        z.push_back(N-y[i+1]);
        if((i+1)*t>=tmax) break;
    }
    ...
    std::ofstream plik ("wyniki1b.dat");
